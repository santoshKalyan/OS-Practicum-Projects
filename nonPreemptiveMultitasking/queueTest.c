...
queue_append(queue_t queue, void* item) {
	...
	if(temp != NULL)
		...
		*/
		if(head==queue) //Empty queue
			{
			printf("\nInserting First Element:%d",*(int *)temp->item);
			temp->next=queue;
			temp->prev=queue;
			...
			{
			printf("\nInserting Element at the End:%d",*(int *)temp->item);
			temp->next=queue;
			temp->prev=tail;
...
queue_iterate(queue_t queue, PFany f, void* item) {
	...
	while(temp != queue)
	{
		...
		returnvalue_PFany=f(temp->item,item);
...
int queue_delete(queue_t queue, void** item) {
	...
	if(prev_temp==queue) {head=prev_temp->next;tail=prev_temp->prev;} // special case of one element
