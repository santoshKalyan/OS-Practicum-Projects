...
extern void pack_unsigned_short(char *buf, unsigned short val);

/* packs a native unsigned integer into 4 bytes in network byte order */
extern void pack_unsigned_int(char *buf, unsigned int val);

/* packs an opaque network address into 8 bytes in network byte order */
extern void pack_address(char *buf, network_address_t address);

/* unpacks a native unsigned integer from 2 bytes in network byte order */
extern unsigned short unpack_unsigned_short(char *buf);

/* unpacks a native unsigned integer from 4 bytes in network byte order */
extern unsigned int unpack_unsigned_int(char *buf);

/* unpacks a network address from 8 bytes in network byte order */
extern void unpack_address(char* buf, network_address_t address);
