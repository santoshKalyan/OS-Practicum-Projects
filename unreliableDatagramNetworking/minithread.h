...
struct minithread
	...
	int status;
	int priority;
	long start_tick;   //when this thread stated
	};
typedef struct minithread *minithread_t;
typedef struct minithread minithread_s;

int idle_proc(void *num);
int final_proc(void *num);
int getnewthreadid();
int flag;
//We aren't explicitly wrapping max value of global_time to 0
//  as, in C, incrementing the max value by 1 will make it 0.
// For eg,
//       short unsigned int global_timer = 65535;
//       global_timer++; //makes global_timer = 0
unsigned long global_timer; 
int schedular_clock_ticks;
